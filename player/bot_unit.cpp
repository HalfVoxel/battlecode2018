...
PathfindingMap BotUnit::defaultMilitaryTargetMap() {
    ...
    if (reusableMaps.count(reuseObject)) {
        ...
        targetMap = enemyNearbyMap * 0.0001 + discoveryMap * 0.0001;
...
PathfindingMap BotUnit::defaultMilitaryCostMap () {
    ...
    if (reusableMaps.count(reuseObject)) {
        ...
        auto costMap = (passableMap + enemyInfluenceMap * 2.0) / (nearbyFriendMap + 1.0) + structureProximityMap * 0.1 + rocketHazardMap * 1000.0;
